function tag(TagName,BortherNumber,AttributeNomber,Attribute,Style,Attribute1,Style1)
        ...
        var tags = [
            ...
            new tag("figure",3,0),
            new tag("img",2,1,"src",""),
            new tag("svg",1,2,"viewbox","0 0 180 320","preserveAspectRatio","none"),
...
function AddNodes(FatherLocation,ChildTag,Number)
{
    ...
    if(tags[Number].BortherNumber > 0)//如果它有超过一个节点，开始递归，为其插入子节点
    {
        AddNodes(FatherLocation,ChildTag,Number++);
    }
    return para;
}
function ContributeDom(flag)
{
    if(flag == 1)
    {
        ...
        {
            var TempLocation = addnodes(NowLocation,tag[i].TagName);//temp是临时的父节点，也就是构造之后的返回节点
            if(tag[i].BortherNumber != 0)//如果有兄弟节点的话
            {
                var TotalBorther=tag[i].BortherNumber;
                for(var j = 0;j < TotalBorther;j++)
                {
                    addnodes(NowLocation,tag[++i].TagName,);//继续构造兄弟节点
                }
            }
            else {//如果没有兄弟节点
                NowLocation = TempLocation;//更新父节点
            }
        }
    }
}
function ContributeTaem(k)
{
    function member(name,school,image)
    {
        this.name = name;
        this.school = school;
        this.image = image;
